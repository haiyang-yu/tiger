package org.tiger.netty.server;
...
/**
 * {@link BaseNettyTcpServer}
 *
 * @author SongQingWei
 * @since 1.0.0
 * 2020-01-14 17:36 周二
 */
public abstract class BaseNettyTcpServer extends BaseService implements Server {

    private static final Logger LOGGER = LoggerFactory.getLogger(BaseNettyTcpServer.class);
    ...
    public BaseNettyTcpServer(String host, int port) {
        this.host = host;
        this.port = port;
    }
        ...
        try {
            ...
            bootstrap.bind(address).addListener(future -> {
                if (future.isSuccess()) {
                    ...
                    LOGGER.info("{} start success on:{}", this.getClass().getSimpleName(), port);
                    if (listener != null) {
                        listener.onSuccess(port);
                    }
                } else {
                    LOGGER.error("{} start failure on:{}", this.getClass().getSimpleName(), port, future.cause());
                    if (listener != null) {
            ...
            LOGGER.error("Server start occur exception", e);
            if (listener != null) {
